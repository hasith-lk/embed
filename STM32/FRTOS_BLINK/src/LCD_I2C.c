...
#include "LCD_I2C.h"
 ...
 {
    i2cwrite (LCD_I2C->_Addr, data | LCD_I2C->_En);   // En HIGH
    i2cwrite (LCD_I2C->_Addr, data & ~LCD_I2C->_En);  // En LOW
 }
 ...
 LCD_I2C_Struct *new_LCD_I2C_Struct(uint8_t lcd_Addr, uint8_t backlighPin, t_backlighPol pol)
 {
    ...
    LCD_I2C_Struct *LCD_I2C = (LCD_I2C_Struct *)pvPortMalloc(sizeof(LCD_I2C_Struct));
